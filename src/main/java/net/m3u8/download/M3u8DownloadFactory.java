...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        private Thread getThread(String urls, int i) {
            return new Thread(() -> {
//                System.out.println("进入下载线程下载：" + i + " : " + urls );
                int count = 1;
...
//                System.out.println("file2:"+file2.getAbsolutePath());
                ...
                try {
                    //从队列中获取byte数组
                    //如果队列中没有byte数组，则阻塞等待（如果队列中有byte数组，则直接获取，不会阻塞
...
//                    e.printStackTrace();
     ...
     * @return 返回m3u8下载实例
     */
    public static M3u8Download getInstance(String downloadUrl) { // 作者原版方法,只能有一个实例
    ...
    public static M3u8Download newInstance(String downloadUrl) { // 多个实例
